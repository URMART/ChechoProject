...
def ingresar(request):
    if request.method == "POST":
        try:
            ...
            request.session["logueo"] = [q.id,q.usuario,q.rol,q.nombre]
            messages.success(request, f'Bienvenido {request.session["logueo"][3] } ')
...
def guardarUser(request):
    try:
        if request.method == 'POST':
            ...
            q=Clientes.objects.create(nombre=nom,apellido=apell,numeroTelefono=numTel,usuario=user,contraseña=contra,foto=file)
...
def guardarCita(request):
    ...
    try:
        ...
        if request.method == 'POST' and login:
            ...
            if login[2]=='cliente':
                user=Clientes.objects.get(id=login[0])
            elif login[2]=='administrador':  
                user=Administradores.objects.get(id=login[0]) 
            elif login[2]=='empleado':  
                user=Empleados.objects.get(id=login[0])   

            empleado=Empleados.objects.get(id=emple)
            servicio=Servicios.objects.get(id=servi)

            cita=Citas.objects.create(fecha=fech, hora=hor, cliente=user, servicio=servicio,empleado=empleado)
...
def cortes(request):
    return render(request, 'portafolio/cortes.html')

def barbas(request):
    return render(request, 'portafolio/barbas.html')

def tinturas(request):
    return render(request, 'portafolio/tinturas.html')
